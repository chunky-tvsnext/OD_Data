...
// console.log('req.body',req.params)
    ...
    request(`http://34.93.117.215:8443/battery/data/${batteryId}`, { json: true }, (err, res, body) => {
            ...
            if (body.battery_log==undefined) { 
                let status = saveNoResponseData(batteryId);
                ress.status(500).send({
                    message: "No data found for this BIN"
                });
...
function saveNoResponseData(data){
...
function getBinNumber(imei_tcu){
    
    BinN.find({"imei_tcu" : `${imei_tcu}`},{binNumber : 1})
    .then(notes => {
        console.log('binNumber',notes)
        return notes;
    }).catch(err => {
        res.status(500).send({
            message: err.message || "Some error occurred while retrieving notes."
...
function getIMEINumber(binNumber){
    return new Promise(function(resolve, reject){
        BinN.find({"binNumber" : `${binNumber}`},{imei_tcu : 1})
        .then(imei_tcu => {
            console.log('imei_tcu',imei_tcu)
            resolve(imei_tcu) ;
        }).catch(err => {
            reject(err)
        });
